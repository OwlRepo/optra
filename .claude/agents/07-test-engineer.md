---
name: test-engineer
description: "Use proactively for the RED round before implementers run and for the post-implementation test review. Writes Jest specs in apps/api and Vitest specs elsewhere, error: > edge: > regression: > happy:, and records RED with bun run tdd:red."
...
- No Playwright in this repo. Browser QA is the orchestrator's `/qa` skill, not a test file.
...
- Layers follow the diff: a unit spec for every changed service, guard, processor, chain, hook or component; an e2e spec when a route's HTTP contract or guard chain changes; a migration test (`packages/db/src/**/*.spec.ts` or an api e2e spec) for any migration.
