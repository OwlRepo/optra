...
**Project:** Optra — a multi-tenant procurement SaaS: buyers connect their own vendor catalogs, purchase orders, and invoices, and Optra matches every PO line against the catalog entry (price, quantity, and product photo) and flags discrepancies before payment, with a citation behind every verdict. It is built on a multi-tenant RAG core that still powers grounded chat over a workspace's own documents, AI ticket extraction, and web-source crawling. **Production software** — this is stated explicitly (multi-tenant workspaces, JWT+OTP auth, rate limits and token budgets, GitHub Actions CI/CD auto-deploying to a live VPS with backups), not inferred.
