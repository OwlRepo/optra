...
describe('GET /api/workspaces/[id]/digest-settings/preview proxy', () => {
  afterEach(() => {
    vi.unstubAllGlobals()
  })

  it('forwards bearer and returns the backend payload', async () => {
    const payload = { emailHtml: '<h2>digest</h2>', slackPayload: { text: 'digest' } }
    const fetchMock = vi.fn().mockResolvedValue(mockBackendResponse(200, payload))
    vi.stubGlobal('fetch', fetchMock)

    const request = new NextRequest('http://localhost:3000/api/workspaces/ws-1/digest-settings/preview', {
      method: 'GET',
    ...
    expect(fetchMock).toHaveBeenCalledWith('http://localhost:3001/workspaces/ws-1/digest-settings/preview', {
      method: 'GET',
