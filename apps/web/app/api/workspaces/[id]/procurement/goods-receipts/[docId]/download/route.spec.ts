...
describe('GET /api/workspaces/[id]/procurement/goods-receipts/[docId]/download proxy', () => {
  afterEach(() => {
    vi.unstubAllGlobals()
  })
  ...
  it('streams the backend download and preserves the filename header', async () => {
    ...
    const request = new NextRequest(
      'http://localhost:3000/api/workspaces/ws-1/procurement/goods-receipts/doc-1/download',
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      'http://localhost:3001/workspaces/ws-1/procurement/goods-receipts/doc-1/download',
  ...
  it('returns 401 without calling the backend when the auth cookie is missing', async () => {
    ...
    const response = await GET(
      new NextRequest('http://localhost:3000/api/workspaces/ws-1/procurement/goods-receipts/doc-1/download'),
