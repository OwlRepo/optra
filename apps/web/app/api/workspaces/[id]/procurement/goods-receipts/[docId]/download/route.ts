...
// the purchase-order and invoice downloads. proxyRaw already forwards
...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string; docId: string }> }) {
  ...
  return proxyRaw(request, `/workspaces/${id}/procurement/goods-receipts/${docId}/download`, { method: 'GET' })
}
