...
const statusVariant: Record<DocumentRow['status'], 'secondary' | 'success' | 'destructive'> = {
  pending: 'secondary',
...
export default function KnowledgeBasePage({ params }: { params: { id: string; kbId: string } }) {
  ...
  const [pendingDelete, setPendingDelete] = React.useState<DocumentRow | null>(null)
  ...
  const hasInFlightRuns = scrapeRuns.some((run) => run.status === 'queued' || run.status === 'running')
  ...
  const handleFileChange = React.useCallback(
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        const created = await uploadDocument(workspaceId, knowledgeBaseId, file)
  ...
  }, [knowledgeBaseId, loadDocuments, pendingDelete, router, toast, workspaceId])
  ...
  const submitScrape = React.useCallback(async () => {
    try {
      ...
      await scrapeSite(workspaceId, knowledgeBaseId, payload)
      ...
      toast({
        variant: 'success',
        title: 'Crawl started',
        description: 'Website crawl queued for processing.',
      })
  ...
  }, [knowledgeBaseId, loadDocuments, loadScrapeRuns, router, scrapeMaxDepth, scrapeMaxPages, scrapeUrl, toast, workspaceId])
    ...
    <PageShell contentClassName="pb-16">
      <AppHeader
        ...
        navigation={
          <>
            {canManage ? (
              <Button type="button" size="sm" onClick={() => setIsScrapeModalOpen(true)}>
                Scrape website
              </Button>
            ) : null}
            ...
            {canManage ? (
              <div className="space-y-4">
                ...
                >
                  ...
                  <span className="text-base font-semibold">Drop a file here or click to upload</span>
                  <span className="mt-2 text-sm text-muted-foreground">Small `.txt` and `.pdf` files are good for quick validation.</span>
                </label>
                <input
                  ref={fileInputRef}
                  id="document-upload"
              ...
              <TableBody>
                {scrapeRuns.map((run) => (
                  <TableRow key={run.id}>
                    ...
                    <TableCell>{`${run.pagesFound}/${run.pagesSucceeded}/${run.pagesFailed}`}</TableCell>
                    <TableCell>{run.startedAt ? new Date(run.startedAt).toLocaleString() : 'Queued'}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
              ...
              <TableBody>
                {documents.map((document) => (
                  <TableRow key={document.id}>
                    ...
                    <TableCell>{document.createdAt ? new Date(document.createdAt).toLocaleString() : 'Just now'}</TableCell>
      ...
      >
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-3">
            <Button type="button" variant="ghost" onClick={() => setIsScrapeModalOpen(false)}>
              Cancel
            </Button>
            <Button type="button" onClick={() => void submitScrape()} disabled={!scrapeUrl}>
              Start crawl
            </Button>
          </div>
        </div>
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-muted-foreground">
            {pendingDelete ? `Delete ${pendingDelete.title}? This removes the source file and its indexed chunks.` : ''}
          </p>
          <div className="flex justify-end gap-3">
