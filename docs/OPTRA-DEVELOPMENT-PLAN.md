...
**Status:** Draft for review — repository-wide audit revision 3 / final code-facts closure
**Prepared:** 2026-09-16
**Repository baseline:** `main`, commit `469317ec9c9d2ca02d4ae0572b5fedc4568af3f2`
**Planning constraint:** no implementation in this artifact; code changes require separate explicit approval.

**Coverage statement:** static repository inventory and relevant source inspection completed across all tracked product, platform, test, configuration, deployment, seed, evaluation, and documentation surfaces. External service behavior, customer workflow truth, and currently unavailable service-backed tests remain explicitly unverified.

## 0. Executive recommendation

Narrow Optra’s product spine to:

`PO + receiving evidence + supplier invoice + vendor price/contract evidence`
`→ normalize → match → verify evidence → exception queue → human decision → audit trail`

The current code already supports a useful first slice: upload PO/invoice files, extract rows, run a manual two-way comparison, create discrepancy flags, ingest vendor catalogs, and manually request catalog matches. The code does **not** yet support the proposed trustworthy control plane: receiving/GRN evidence, versioned pricing, persistent UOM rules, comparison runs, automatic post-upload matching, calibrated confidence, human decisions, or an audit trail.

Recommendation: build the evidence and decision spine first. Keep messaging, BIR/tax automation, broad RAG/“AI operations,” ERP integrations, and payment automation behind validation gates.

## 1. Planning rules

This document uses three labels:

- **FACT** — verified from repository source, tests, configuration, or observed command output.
- **PROPOSED** — recommended product or engineering decision; not yet implemented.
- **UNVERIFIED DEPENDENCY** — required input or decision that cannot safely be invented.

Hard stops:

1. Do not design the final schema around invented customer document fields.
2. Do not treat an LLM score as an approval decision.
3. Do not claim three-way matching until receiving evidence is modeled and tested.
4. Do not add tax, messaging, ERP, or payment behavior without validated contracts and acceptance fixtures.
5. Do not run production migrations from `db:push`; use reviewed Drizzle migrations.
6. Do not expose raw document contents in logs, URLs, or model prompts beyond the approved data-processing policy.

## 2. Requirements restated

The requested outcome is a full development plan for the existing Optra codebase, based on exhaustive inspection rather than assumptions, delivered as one revisable Markdown file.

The product outcome being planned is:

- A hardware-company team uploads or receives procurement evidence.
- Optra extracts structured records while retaining source provenance.
- Optra compares ordered, received, invoiced, and contract-priced values.
- Optra surfaces only explainable exceptions.
- A human reviews evidence, records a decision, and leaves an auditable trail.
- Vendor history and operational insights compound from reviewed outcomes.
...
**FACT:** `apps/api/src/app.module.ts:28-60` registers auth/workspace, procurement, catalog, structured query, storage, AI, and other application modules. Procurement and catalog queues run in the API process.

**FACT:** current relevant web navigation is defined in `apps/web/src/components/workspace-nav.tsx:11-25` and includes Vendors, Purchase Orders, Discrepancies, and Catalog Matches. The primary mobile tabs remain Overview, Chat, and Knowledge.
...
- Manual web selection and compare action: `apps/web/app/workspaces/[id]/procurement/page.tsx:223-251`.
...
**FACT:** extraction normalizes numeric values and allows nullable fields. Confidence is nullable and bounded to 0–1, but source page/field coordinates are not persisted (`packages/ai/src/chains/procurement-extraction.ts:146-213`).
...
**FACT:** the current upload service classifies every non-PDF upload as `sourceKind: 'csv'` (`apps/api/src/procurement/procurement-documents.service.ts:22-31`). During XLSX parsing, the processor converts the first sheet to CSV and overwrites the original object at the same `storageKey` (`apps/api/src/procurement/procurement-parse.processor.ts:89-103`). This exists because the later DuckDB comparison path reads CSV, but it destroys the original XLSX bytes and conflates source format with derived parse format. The current implementation therefore cannot support immutable source evidence, original-format download, or revision-safe reprocessing for XLSX without a storage/model change.

**Required fix before provenance claims:** preserve the immutable uploaded object and store any normalized CSV as a separate derived artifact or persist normalized rows without mutating the source object. Record original format, derived format, parser/version, and the relationship between them after Gate 0 accepts the retention/provenance contract.
...
|---|---|---|
| `purchase_orders` | workspace, name, PO number, currency, storage key, source kind, parse lifecycle, row count/error | vendor relation, provenance, terms, receiving links |
| `invoices` | workspace, invoice number, currency, storage key, source kind, parse lifecycle | vendor relation, tax/terms, receiving links, provenance |
...
**FACT:** catalog upload/scrape/search/verify/dismiss/photo routes exist in `apps/api/src/catalog/catalog.controller.ts:97-218`.
...
**FACT:** no automatic post-upload match worker exists. `apps/api/src/catalog/catalog.module.ts:15-34` registers parse and scrape queues, but no match queue.

**FACT:** model output is boolean match, 0–1 score, and reason (`packages/ai/src/chains/catalog-match.ts:189-289`). There is no calibration dataset or proven threshold. The current score must not be treated as approval authority.
...
**FACT:** services commonly enforce workspace predicates and workspace existence checks. Example: `apps/api/src/catalog/catalog-match.service.ts:118-139` and `apps/api/src/catalog/catalog-documents.service.ts:80-95`.
...
**Important:** the API/AI failures do not prove product regressions. They prove that the required database, S3, and network-backed test dependencies were unavailable for this planning run. Do not claim the complete suite is green.
...
## 5. Gate 0 — evidence and contract validation
...
### 7.2 Workspace isolation and permissions
...
Before production scale, evaluate worker separation. Current source registers processors in the API process (`apps/api/src/app.module.ts:28-60`); `docs/PRODUCTION-READINESS.md` identifies this as an operational risk.
...
### 10.2 Database and migration tests
...
### 10.4 Matching/comparison matrix
...
### 10.5 Security and tenancy tests
...
The source already has a global exception filter in `apps/api/src/main.ts:7-16`; documentation rows that disagree must be reconciled before using them as acceptance criteria.
...
**PROPOSED:** progressive rollout by workspace and feature flag:

1. internal fixtures only;
2. one design-partner workspace with redacted/consented data;
3. shadow comparison beside the current manual two-way flow;
4. reviewer-only evidence queue;
5. measured expansion to additional workspaces;
6. integrations only after repeated successful core runs.

During shadow mode, do not alter payment or ERP state.
...
Leave unresolved until reviewed:

| Decision | Status | Required evidence/owner |
|---|---|---|
| Primary target customer segment | UNVERIFIED | Customer interviews and document access |
| Authoritative receiving artifact | UNVERIFIED | Customer operations/finance |
| Canonical PO/invoice/receiving linkage | UNVERIFIED | Real document fields |
| Price authority and semantics | UNVERIFIED | Contract/accounting policy |
| UOM conversion policy | UNVERIFIED | Procurement/warehouse owner |
| Review/decision vocabulary | UNVERIFIED | AP/procurement approval policy |
| Exact role permissions | UNVERIFIED | Workspace/customer security owner |
| Required intake channels | UNVERIFIED | Observed user workflow |
| Tax scope | UNVERIFIED | Tax-professional signoff |
| Retention/export/deletion | UNVERIFIED | Customer/legal/security requirements |
| Deployment/service topology | UNVERIFIED | Current runtime/operator |

## 17. Review checklist before implementation approval

- [ ] Gate 0 sample documents attached or location recorded.
- [ ] Customer document taxonomy accepted.
- [ ] Receiving source and linkage fields accepted.
- [ ] Price/UOM semantics accepted.
- [ ] Decision states and role matrix accepted.
- [ ] Data retention/export/deletion accepted.
- [ ] PII/model-processing posture accepted.
- [ ] Golden extraction and comparison fixtures defined.
- [ ] Migration strategy reviewed from current commit.
- [ ] Queue/idempotency/retry behavior reviewed.
- [ ] API contracts reviewed.
- [ ] UI states and evidence requirements reviewed.
- [ ] Security/IDOR test matrix accepted.
- [ ] Production readiness blockers assigned.
- [ ] Phase 1 acceptance gate signed off.
- [ ] No implementation starts before explicit approval of the reviewed plan.
...
## 19. Repository-wide 100% coverage audit
...
**FACT:** the repository baseline contains 916 tracked files, excluding this untracked plan artifact. The following top-level inventory is disjoint and reconciles exactly:
...
| `docs` | 22 | Product, operations, design, and AI engineering docs |
...
| **Total** | **916** | Exact `git ls-files` count at the baseline commit |
...
**FACT:** 31 schema files define 30 PostgreSQL tables and 22 numbered SQL migrations with 22 snapshots. The schema contains 18 enum declarations.

Tables grouped by responsibility:

- **Identity/tenancy:** `users`, `workspaces`, `workspace_members`, `invitations`, `otps`, `refresh_tokens`.
- **Knowledge/RAG:** `knowledge_bases`, `documents`, `chunks`, `chat_sessions`, `chat_messages`, `chat_cache`, `chat_query_metrics`, `saved_refined_messages`.
...
**FACT:** `.github/workflows/deploy.yml` runs on pushes to `main` and manual dispatch. It SSHes to the VPS, checks out code, creates a PostgreSQL dump backup, builds API/web images, recreates the stack, runs health checks, and prints logs on failure. It is not a pull-request quality gate.
...
## Receiving Exceptions & Edge Case Workflows (Site & Warehouse Reality)

**Status:** proposed product extension; not implemented in the current repository.
...
## 20. Final code-facts coverage closure — 2026-09-16
...
**FACT:** the audit was run against `main` at commit `469317ec9c9d2ca02d4ae0572b5fedc4568af3f`. No application source, schema, migration, test, configuration, or deployment file was changed during this audit; only this untracked Markdown plan is being revised.

**FACT:** `git ls-files` returned 916 tracked paths before this plan is added to version control. The top-level inventory is disjoint: `apps` 505, `packages` 204, `scripts` 35, `docs` 22, `.ai-engineering` 44, `.claude` 70, `graphify-out` 9, `docker` 7, `.github` 1, and 19 root files.
...
| Original source and derived parse | Non-PDF uploads are marked `sourceKind: 'csv'`; XLSX is converted from its first sheet and written back to the same storage key because DuckDB later reads CSV. This is an implementation shortcut for the current comparison engine. | Keep immutable original bytes; store derived CSV/normalized rows separately; record original format, derived format, parser/version, and source-to-derived relation. | Required before evidence/revision claims; retention and reprocessing contract is UNVERIFIED. |
...
1. **XLSX source mutation:** `apps/api/src/procurement/procurement-documents.service.ts:22-31` collapses `.xlsx` into `sourceKind: 'csv'`; `apps/api/src/procurement/procurement-parse.processor.ts:89-103` overwrites the uploaded object with converted CSV. The reason is downstream DuckDB compatibility. The fix is an immutable-original plus derived-artifact relationship, not a UI-only label.
...
5. **Destructive comparison refresh:** a rerun deletes the current flag set for a PO/invoice pair and inserts a new set. This was sufficient for the prototype’s idempotent current-state behavior; it is insufficient for historical audit, source revision comparison, or reproducing a prior decision.
...
### 20.6 Final implementation-readiness verdict
...
Proceed, but with a narrower product contract:

1. Validate real hardware-company documents and workflows.
2. Add receiving evidence, provenance, versioned pricing, UOM rules, comparison runs, and human decisions.
3. Make the review queue the first polished product surface.
4. Use AI to extract, rank, and explain—not to authorize money.
5. Build vendor intelligence from reviewed outcomes.
6. Gate messaging, tax, ERP, and broader AI features on proven demand and verified contracts.
7. Keep this file as the living plan; revise it after every accepted discovery decision or codebase change.
