...
**FACT (corrected 2026-09-21 during S3b — CONTEXT DRIFT):** this previously read that `ComparisonService` loads child line items using only `purchaseOrderId`/`invoiceId`. **That is no longer true and has not been since S0a.** `compare()` now filters both child reads by workspace — `and(eq(poLineItems.workspaceId, workspaceId), eq(poLineItems.purchaseOrderId, po.id))` and the invoice equivalent (`apps/api/src/procurement/comparison.service.ts:169-176`), with a comment naming it as defense in depth. The parent checks in `loadReadyPo`/`loadReadyInvoice` are unchanged. The standing requirement still holds for new code: enforce both the parent relationship and the workspace scope. S3b adds the same discipline to caller-supplied ids on upload (see `docs/ai/risk-register.md`, "Cross-Workspace Ids In Request Bodies").
...
| D4 | There is no procurement source-document download or view route in the API, the BFF, or `apps/web/src/lib/api/procurement.ts`. | cited | S4 — **done 2026-09-20**: `GET .../purchase-orders/:docId/download` and `.../invoices/:docId/download`, member-readable, always `attachment` + `application/octet-stream` + `nosniff`. BFF routes, a client function on the existing `fetchDownload` helper, and a per-row control on the procurement page. |
...
| **S2** Decisions + audit ✅ 2026-09-20 | Deep (schema) | Migration **0023** adds append-only `discrepancy_decisions` (workspace, flag, run, actor, `actor_role` captured at decision time, outcome, required note). `POST .../discrepancies/:flagId/decisions` (owner/admin, note required) and `GET` the same path (member-readable, oldest-first). `PATCH dismiss` keeps its signature and now writes one `false_positive` decision with an authored system note, inside the same transaction as the status change; a re-dismiss adds no second decision. **Design note:** every outcome closes the flag and `discrepancy_flag_status` stays two-state — giving it four new values would be the repo's first `ALTER TYPE` on a forward-only deploy, for a display distinction. | B5 (history) | S1 |
| **S3a** Line-item provenance ✅ 2026-09-20 | Deep (schema) | Migration **0024** adds five nullable columns to both line-item tables — `source_sheet`, `source_row`, `uom`, `extraction_confidence`, `extractor_version` — written by the parse worker. A CSV/XLSX row records where it sat (`source_row` is the real file line, which required turning Papa's `skipEmptyLines` **off** so the index still maps to the file; blank rows are still dropped by `isEmptyLineItem`); an XLSX row records its sheet; a PDF row records the model's confidence, promoted out of the `raw_row` blob where it was unqueryable, plus an `EXTRACTOR_VERSION` stamp. Existing rows are backfilled from `raw_row->>'confidence'`, guarded by `jsonb_typeof` so a non-numeric value is skipped rather than throwing. **`source_page` was dropped, deliberately:** the text path flattens a PDF to one string and the vision path sends every page in one message, so no honest page number exists without per-page model calls — shipping a sixth column nothing writes is the defect this slice exists to close. **`uom` is captured, never converted** (POLICY v1 #4); its alias list excludes `unit`/`units` because `units` already means quantity, and stealing it would silently corrupt quantities. | dead/absent provenance columns (§3 `:319-320`) | S0b |
| **S3b** Header enrichment + linkage | Deep (schema) | `po_number`/`invoice_number`/`currency` captured at upload (nothing in the repo extracts document-level metadata, and POLICY v1 #2 wants linkage explicit anyway), PO→`vendorId`, and an explicit invoice→PO link. Needs upload-form changes. | header dead columns, POLICY v1 #2–#3 | S3a | **DONE 2026-09-21**, migration `0025_broad_invisible_woman.sql`, branch `feat/s3b-header-linkage`. Owner decisions taken during planning: the invoice→PO link and the PO's `vendorId` are both **required at upload** (POLICY v1 #2/#3 read that way, and S5's GRN inherits the shape) — which forces PO-before-invoice ordering, handled by an explanatory empty state in the modal rather than an API error; `compare()` **enforces** the link only when it is non-null, so every pre-0025 document still compares; the B16 vendor deep link **defers to S7**, which rewrites that page. Both columns are nullable regardless of the form (hard stop #7). `currency` is validated ISO 4217 **and uppercased in the service** — `IsISO4217CurrencyCode` is case-insensitive, so validation alone would have let `usd` and `USD` both reach the column and manufactured the exact false mismatch S6 exists to avoid. No uniqueness constraint on document numbers (`vendors` has none on name either, and revised invoices legitimately reuse a number). |
| **S4** Source evidence route ✅ 2026-09-20 | Standard | Two member-readable `@Res()` routes returning the original uploaded bytes. **Copied the knowledge-base download, not `getItemPhoto`:** the photo route is deliberately inline with a raster allowlist, which is the wrong shape for `.csv`/`.xlsx`/`.pdf` — these are always `attachment` + hard-coded `application/octet-stream`, so the stored content type is never echoed and no allowlist is needed, plus `X-Content-Type-Options: nosniff` (otherwise set only by `docker/Caddyfile`, so absent in local dev). `list()` gained a derived `hasSourceFile` boolean — `storageKey` is nullable and the key itself never leaves the API. Also extracted the `Content-Disposition` filename sanitizer, which existed as two independent copies, into `apps/api/src/common/http/content-disposition.ts`. | D4 | S0b |
| **S5** Goods Receipt domain | Deep (schema) | Add GRN header and line tables mirroring PO/invoice, including qty received/accepted/rejected and `uom`. Reuse the upload→parse→queue template (`procurement-parse.service.ts:46-80`). Require an explicit PO link. | receiving gap | S3 | **DONE 2026-09-22**, migration `0026_nostalgic_black_widow.sql`, branch `feat/s5-goods-receipt`, two commits. Ingest only — `comparison_runs`/`discrepancy_flags` are untouched and S6 owns that migration (owner decision), because a column with no writer is the defect S3a/S3b removed. Quantities are **three named columns** (`quantity_received`/`quantity_accepted`/`quantity_rejected`), not a reused `quantity`, so no column means ordered on one table and received on another. All three are **nullable with no default: null is "not stated", never zero** (§1B `:745`, POLICY v1 #14) — S6 must preserve that or it will report rejections that never happened. `goods_receipts.purchase_order_id` is **NOT NULL and non-unique**: the table is new so POLICY v1 #2's explicit link lives in the schema, and POLICY v1 #14 allows many receipts per PO. No vendor (POLICY v1 #3, inherited via the PO) and no currency (a receipt records what arrived, not what it cost). `backordered`/`unknown` from §1B `:741` are **not** added — POLICY v1 #1 does not declare them and hard stop #1 forbids it; `:675` rules POLICY v1 supplies the answer. **CSV/XLSX only, PDF deferred with reason:** the extraction chain prompts for "purchase order or invoice" and `ExtractedLineItem` has no received/accepted/rejected, so a PDF receipt would land one quantity and silently lose the acceptance data. **Prerequisite work this slice absorbed:** ~15 sites branched `kind === 'purchase_order' ? … : …` with the else-branch meaning invoice, and the repo had no exhaustive switch anywhere — widening the union naively would have written receipts into `invoices`. They became switches over a new `assertUnreachable` (`procurement-kind.ts`) **before** the union grew, verified by running the whole suite with zero test edits. **Known limit, recorded in `docs/ai/risk-register.md`:** that guard only covers code that branches. `ProcurementParseService.reconcile` is a sequence of per-table scans and the per-kind controller routes are repetition, so neither errors — the reconcile scan was added by hand. |
| **S6** Three-way comparison | Deep | Add run mode `three_way`. Sum accepted qty across linked GRNs (POLICY v1 #14). Add new flag types through an additive enum migration. UOM and currency mismatch → `needs_review`. Golden three-way fixtures. | three-way gap | S1, S5 | **DONE 2026-09-22**, migration `0027_common_darkstar.sql`, branch `feat/s6-three-way-comparison`, two commits. **`needs_review` is a category, not a value** (owner decision): the enum gained **four** values — `short_receipt`, `invoice_exceeds_received`, `uom_mismatch`, `currency_mismatch` — so the *reason* a line needs review survives and S7's queue can filter on it. The last two are the needs-review pair, recognisable mechanically because POLICY v1 #4 and #6 compute **no delta**. This is the repo's **first `ALTER TYPE … ADD VALUE`**, sanctioned by §6.7 `:1132`. The migration **adds values and never writes one**, which is load-bearing: `drizzle-orm/pg-core/dialect.js` wraps ALL pending migrations in ONE transaction, so on a fresh database the type is born inside it and a new value is usable, while on the deployed database it is not — a migration that wrote one would pass every local check and fail only in production. Verified on both paths. drizzle-kit emits `ADD VALUE` unguarded, so `IF NOT EXISTS` was added by hand to match every `CREATE TYPE` here. **A run records its receipts in a join table** (`comparison_run_goods_receipts`), not a singular column: POLICY v1 #14 sums across *every* linked receipt and §1E `:786` wants the source document IDs — this supersedes an S5 note proposing `comparison_runs.goods_receipt_id`, corrected in the risk register and repository map. `discrepancy_flags` gained `received_value`, so one flag row carries ordered/received/billed together and the CASE branch order picks only the **label**, never what the reviewer can see. **The invariant the slice is built on:** a NULL accepted quantity means "not stated", never zero — a bare `SUM()` returns a partial total and would accuse a supplier who delivered correctly, so the receipt aggregate reuses the existing `COUNT(*) = COUNT(TRY_CAST(...))` guard and every branch tests `IS NOT NULL` first. `uom_mismatch` outranks every numeric branch (a quantity difference measured across different units is not a number anyone can act on) and also fires when one document's own duplicate lines disagree, since the engine sums them. `currency_mismatch` is the only flag not derived from a matched line: header-level, computed in TypeScript, one per run, no line references. **Known tensions recorded, not implemented:** §7.4 `:1209` asks for conversion under an approved rule and `:1210` reads "UOM unknown" as needs-review — POLICY v1 #4 and `:675` win, so units are never converted, and an *unstated* unit is never a mismatch (most rows carry none; treating absent as different would put every comparison into review). **Two bugs the enforced red-first order surfaced:** `Papa.unparse([])` writes no header, so an empty `grn.csv` broke *every* comparison including two-way ones; and three "passing" three-way tests were vacuous against the old two-sided engine. **A third came from diffing the seed against the engine:** a receipt line's `uom` was keyed on its row number within that receipt, so two deliveries for one PO disagreed about the unit — the engine flagged the demo data, and was right. |
| **S7** Review queue UI | Standard | Turn the discrepancies page into a queue with decision actions, run history, and evidence links. Paginate the API and web lists. Match the design tokens (`DESIGN.md`). | D6 | S2, S4 | **DONE 2026-09-23**, no migration, branch `feat/s7-review-queue`, two commits. **Reclassified Deep during planning**, not Standard: it changes a public response shape and rewrites the query every discrepancy read depends on. **Commit 1 — pagination.** `listFlags` moved to the offset envelope (`docs/ai/contracts/api-contracts.md:27-44`), which is a breaking change from the bare array; all nine consumers moved with it. The six stat cards now read server-computed `counts` over the whole filtered set — a client memo over the visible page would have reported "1 price mismatch" on a page showing one of forty — and `total` is derived by summing `counts`, so the cards can never disagree with the list. "Current flags" moved from a map plus an in-memory filter into a SQL predicate (`currentFlagScope`), because a filter applied after the query cannot be paginated; the workspace-wide read of every succeeded run went with it, and the four S1 current-run tests passing **unedited** is the proof the rewrite changed nothing. **The ordering fix is load-bearing:** `compare()` writes every flag of a run in one transaction, so `created_at` is `now()` evaluated once and they share it exactly — ordering on it alone is not a total order, and OFFSET over it repeats some rows and drops others. Now `(created_at, id)`. **Commit 2 — decisions and run history.** `GET .../comparison-runs` (member-readable, pair-filterable, served by existing indexes) makes runs discoverable for the first time since S1; `lastError` is returned because S0a made it a client-safe reference. Decisions gain `actorEmail` (LEFT join on `users`; `actorRole` stays captured-at-decision-time). A `Modal size="xl"` review panel — no new overlay primitive, per `DESIGN.md:53` — carries the stored evidence, the pair's runs, the decision trail, and POLICY v1 #7's four outcomes with its required note. **The decision spine had shipped in S2 with zero callers**; this is its first. `PATCH .../dismiss` stays (POLICY v1 #7) and its `false_positive` decision is now visible. **Closed an unrelated gap found on the way:** procurement role gating had never been tested — every 403 came from `WorkspaceMemberGuard` because the fixture was always an outsider, so `RolesGuard` had never run in that file and a member able to decide would have passed all 52 e2e tests. **Deferred with reasons** (`docs/ai/risk-register.md`): the §2C **evidence drawer** — no route returns a line item or its `rawRow`, and S3a deliberately dropped `source_page`, so "reach the source page/row" is only half reachable and needs its own design pass; **document-list pagination (D6-remainder → 2A)** — `procurement/page.tsx:804,878` uses `purchaseOrders` as the `<Select>` source for S3b's required upload links, so paging it would make any PO past page one unlinkable, a regression worse than the unbounded read; **`Modal` focus trap and body scroll lock** — real gaps, six-plus consumers, would swamp a feature diff; and §2B's **green "evidence agrees" state**, which has no row to render because nothing persists a per-line agreed result. |
| **S8** Auto orchestration | Deep | After parse succeeds and the linked counterpart is ready, enqueue a comparison run with a deterministic jobId. Add procurement event types (additive enum) via `EventsService.record` (`events.service.ts:17`). | orchestration gap | S1, S0c | **DONE 2026-09-23**, migration **0028** (enum values only), branch `feat/s8-auto-orchestration`, three commits. Ships behind `PROCUREMENT_AUTO_COMPARE_ENABLED`, **default off** — a run is permanent evidence and POLICY v1 #9 then refuses to hard-delete any document it referenced, so the first deploy of something that writes runs unattended should not be the first time anyone watches it. Default-off is also why every pre-existing e2e run-count assertion stayed green unedited. **Commit 1 was not in the plan:** `compare()` read goods receipts without filtering on the receipt's own `status`, while the PO and invoice sides had been status-guarded since S0a. A failed re-parse leaves the previous attempt's line rows in place, and a stale low accepted quantity produces `short_receipt`/`invoice_exceeds_received` — an accusation against a supplier who delivered correctly. Latent on the manual path; landed first, in isolation. **The job id carries the input version** (`procurement-compare:{po}:{invoice}:{newestInputUpdatedAtMs}`) rather than being fixed. A fixed id is not a weaker deduplication but a wrong one: while a job sits delayed Bull hands that same job to the next `add`, so a second document's parse is swallowed by a job that already read the old state — a missing run, not a duplicate. **The freshness check** compares inputs against the run's `started_at`, strictly, and includes `strategy_version` and the PO's `done` receipts. `finished_at` would call a torn read current; `strategy_version` had shipped in S1 as a default with no writer, so `compare()` now writes it. **Events:** `comparison_flagged` (not `comparison_completed` — owner decision; an event named for completion that fires only when flags were found is a lie) when a run finds something, `comparison_failed` when one fails, both fire-and-forget against the run id. A clean run says nothing, because `unreadCount` has no per-type filter and `markSeen` is one global watermark. **Deferred, with reasons:** (a) no `queue_job_id`/`enqueued_at` lifecycle columns — a job lost between enqueue and start writes no row, recovered by the next parse or the manual button, which §1F keeps; (b) `comparison_run_status` keeps `'queued'` unused, deliberately, since automation writes no row until the job starts; (c) the stale-run sweep does not raise events — it closes abandoned runs for the manual path too, and whether a manual failure belongs in a shared feed is its own question; (d) per-type unread counts stay out of a procurement slice; (e) §1F's link-suggestion, normalization, catalog-candidate and visual-verification jobs depend on contracts that do not exist. **Nothing auto-approves.** §1F: automatic processing stops at needs-review. |
| **S9** Price + vendor history | Deep | Versioned price and contract evidence (rev 3 §6 1C), then the vendor intelligence surfaces (rev 3 Phase 3). | price gap | S6 | **DONE 2026-09-23**, migrations **0029-0033**, branch `feat/s9-price-vendor-history`, eight commits. **Started on a live UNVERIFIED DEPENDENCY** (§6 1C:759 forbids assuming net/gross, freight, tax, rebates or tiering while §7.4:1207 demands a "contract price expired" exception) and five owner decisions resolved it: both halves with price first; **the approved PO unit price stays authoritative** so POLICY v1 #5 is untouched and `price_mismatch` keeps its meaning; store lean; fix the masked-price defect first; and ask for the order date at upload. **Commit 1 was not in the plan:** the `CASE` ladder returns its first match, so a line whose quantity AND unit price both differed was labelled by the quantity and the price difference was recorded nowhere. The engine had always computed it. Fixed in the writer — **`COMPARISON_SQL` is unedited across all of S9**, which is what makes "the PO-vs-invoice verdict cannot have moved" checkable rather than promised. **The contract check runs in TypeScript**, beside `currencyMismatchFlag`: a first-match `CASE` is a classifier and cannot carry a finding whose truth does not depend on its neighbours. It runs over the purchase order's own lines, not engine rows, because `COMPARISON_SQL` ends `WHERE flag_type <> 'match'` and a line the invoice matches perfectly is exactly when being off contract still matters. A line can now carry two flag rows — different counterparties, different remedies, and S2 decisions are per flag. **`vendor_price_terms`** is append-only with half-open windows; "at most one open term" is enforced in the service because drizzle-kit 0.20 silently drops `.where()` from a unique index and emits a FULL one, which would reject every superseded row. **Deferred, with reasons:** a tolerance on contract variance (POLICY v1 #13 mandates exact match — the likeliest real-world need in the slice); aliases and substitutions (POLICY v1 #3, and they would change the match key, hence every existing flag's identity); exports (POLICY v1 #9, §9.1); scorecards and ranking (Phase 3's own rule bars a ranking becoming a penalty without customer approval, and there are no customers); `price_type`, tiers and tax/freight flags (POLICY v1 #6); contract prices as a fourth document kind; a stable intra-run flag ordinal; currency and UOM conversion. **Plan-doc amendment owed:** Phase 3:965 asks for "contract-versus-**invoice** variance" while the owner's decomposition makes it contract-versus-**PO**. The capability is delivered either way — if PO and invoice agree but both are off contract, `contract_price_variance` catches it; if the PO is on contract and the invoice is not, `price_mismatch` already did — but the wording no longer describes what ships. |
...
- **S1**:
  ...
  - **S3a note:** the plan previously said S3 closes "A6 (dead columns)". That is a dangling id — the only A6 in the repo is `PRODUCTION-READINESS.md:18`, the global exception filter, already closed. The real evidence is §3's own table at `:319-320`, and the S3a/S3b rows now cite it.
...
2. **Child-query workspace hardening — DONE (verified 2026-09-21).** Both child reads in `compare()` carry the denormalized workspace predicate (`comparison.service.ts:169-176`). This item is closed; the text above it in §3.6 was stale and has been corrected.
