...
| B6 | **Inconsistent semantics between upload, seed, and engine.** | cited | S0b (live `sourceKind` is truthful: header `csv`/`xlsx`/`pdf`, lines `csv`/`xlsx`/`pdf-extraction`) — **done 2026-09-20**; S1 (seed delta/values and seed `sourceKind`) open |
