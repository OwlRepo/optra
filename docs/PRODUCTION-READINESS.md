...
| A6 | **Global exception filter** | *(Corrected 2026-09-20: done. `apps/api/src/main.ts:11` installs `AllExceptionsFilter`. The open gap is procurement returning raw engine/parse text; see `docs/OPTRA-DEVELOPMENT-PLAN.md` §4 B17.)* ~~None; relying on Nest defaults.~~ | Add a global filter: safe client messages, no stack/internal leakage, structured error logging. | 🟠 | Prod |
...
| C7 | **Prod stack dry-run** | ✅ Updated 2026-07-08. Well past "not done": a live shared-VPS deploy exists (`mnemra.tyvera.app` *(corrected 2026-09-20: per `docs/ai/risk-register.md:410` that domain is a separate live app; this project's domain comes from the VPS `.env` `DOMAIN` (`docker-compose.prod.yml:87`), which `risk-register.md:403` gives as `optra.tyvera.app`)*), `.github/workflows/deploy.yml` auto-deploys push-to-`main` with DB backup + healthcheck-poll + S3 round-trip, and 4 real Docker/duckdb production bugs have already been found and fixed live on that deploy this week (see `docs/ai/risk-register.md` "Structured SQL Execution" Docker gaps #1-4, and the `.dockerignore`/`coverage` route-folder collision bug under "Production Deployment"). | Row kept 🟠 not ✅ because one 2026-06-30-era gap is still genuinely open: no automated Postgres/SeaweedFS backup+restore test (C1). *(F3 closed 2026-09-20 (S0d): a bad `main` push no longer auto-deploys — `deploy` is gated on the `ci` job passing on that commit.)* | 🟠 | Prod |
...
| F3 | **CI** | **Done 2026-09-20 (S0d).** `deploy.yml` now carries a `ci` job — Node 22 from `.nvmrc`, Bun 1.2.22, the dev compose stack, `db:migrate`, `type-check`, `lint`, and all six unit suites (1297 tests) — and the `deploy` job runs only behind `needs: ci` plus `if: github.ref == 'refs/heads/main'`. Built as **one** workflow, not the separate PR-triggered file this row originally proposed: a second workflow chained by `workflow_run` executes the *default branch's* copy rather than the pushed commit's, so the gate could be bypassed by the very PR that changes it. ~~Updated 2026-07-08 (stale row — `.github/workflows` is no longer absent, but it is a deploy pipeline, not a test-gate).~~ | Residual, not blockers: the gate does **not** run `apps/api` e2e (needs a `@repo/*` build and shares one DB with the unit suites), and `paths-ignore` skips both jobs on a docs-only push, so `workflow_dispatch` is the way to force a deploy after one. | ✅ | — |
