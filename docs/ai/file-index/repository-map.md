...
| `apps/web/app/workspaces/[id]/knowledge-bases/[kbId]/page.spec.ts` | jsdom coverage for document list/upload/poll/delete flows plus crawl-status and scrape-modal regressions | Documents Frontend | Standard | Uses fake timers to verify polling cleanup; covers scrape autofocus/focus retention and `In progress` + labeled crawl counts |
...
| `packages/ai/src/chains/index.ts` | Workspace-scoped RAG answer generation entrypoint; straight-line path by default, conditional LangGraph path behind env flag | Chat / RAG | Deep | `answerQuestion()` now hydrates both document and ticket citations from shared chunk retrieval; `askQuestion()` remains back-compat wrapper. 2026-07-03: routes simple vs complex (`classifyQuery`, `SIMPLE_QUERY_CHUNK_LIMIT`) — simple queries take the light path (fewer chunks, no rewrite/grade) even when graph is on; accepts + threads `precomputedEmbedding` and `RetrievalFilters`; context via `buildEvidencePack()`; answer model via `resolveModel('answer')` |
| `packages/ai/src/chains/graph.ts` | Conditional LangGraph miss-path orchestration: retrieve, score-route, rewrite loop, fallback, optional self-grade/regenerate | Chat / RAG | Deep | Uses retrieval score threshold + rewrite cap; feature flag defaults off; shares mixed document/ticket citation hydration with straight path. 2026-07-03: confident answers stream via `prepareStream`→`streamAnswer` (no buffering); self-grade gated by `SELF_GRADE_MIN_SCORE`; state splits `originalQuestion`/`retrievalQuery`; reuses `precomputedEmbedding` on first retrieval; per-role LLMs (`resolveModel`) |
| `packages/ai/src/chains/models.ts` | Per-task model resolver: `OPENAI_ANSWER/REWRITE/GRADE/EXTRACTION_MODEL` → `OPENAI_CHAT_MODEL` → `gpt-4-turbo` | Chat / RAG | Standard | Added 2026-07-03; unit-tested in `models.spec.ts` |
...
| `packages/ai/src/chains/index.spec.ts` | Unit coverage for deduped source mapping and empty-retrieval fallback in straight-line chat chain | Chat / RAG | Standard | Mocks vector search, DB lookup, and LLM stream; covers mixed document/ticket citations |
| `packages/ai/src/chains/graph.spec.ts` | Unit coverage for LangGraph routing: direct generate, rewrite retry, fallback, and self-grade regenerate | Chat / RAG | Standard | Mocks vector search, DB lookup, and LLM calls; covers ticket citation hydration |
