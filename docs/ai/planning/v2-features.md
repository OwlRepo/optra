...
- S1+F1 (structured CSV querying): **implemented 2026-07-08**, backend and frontend complete, tested (datasets table+migration, dataset upload+profiling pipeline incl. XLSX-to-CSV conversion, hardened DuckDB execution engine, text-to-SQL chain, structured intent routing in ChatService, 4-state result contract rendered distinctly on the web chat page plus a dedicated Datasets management page). See `docs/ai/module-ownership-map.md` "Datasets / Structured Query" row and `docs/ai/risk-register.md` "Structured SQL Execution" for full detail. **Docker gap RESOLVED 2026-07-08:** `duckdb@1.4.4` has no Linux ARM64 prebuilt binary; fixed by pinning `platform: linux/amd64` on the `api` service (both compose files) — no-op on the Hetzner CX21 VPS (native x86_64), forces amd64 emulation on Apple Silicon dev machines.
- F2 (ticket trends, 2b): **implemented 2026-07-08**. Migration `0016` (tickets.category/resolvedAt/assigneeId, additive). `answerFromTickets()` reuses the DuckDbQueryService engine unchanged via a Drizzle-export-to-CSV path. `category` populated deterministically from `productArea` (no extraction-prompt change).
- S2+F3 (scheduler substrate + runbook/SOP freshness detector): **backend implemented 2026-07-08**, tested. Migration `0017` (`background_runs`, `document_review_flags`, additive). `FreshnessTickProcessor` is this repo's first scheduler — one repeatable Bull job (weekly) fans out one per-workspace job. Freshness comparison method decided at this pass (was an open point): embedding-distance heuristic via the shared `TicketDocCoverageService.findGaps()` primitive (cosine over existing chunk embeddings, threshold `FRESHNESS_MIN_SCORE=0.5`, lookback `FRESHNESS_LOOKBACK_DAYS=90`) — zero new LLM cost, matches this batch's cheap-first design; LLM-compare remains the documented fallback posture if the heuristic proves too noisy. S4 Insights shell (tab 1, freshness flags) ships backend+frontend, tested. See `docs/ai/module-ownership-map.md` "Insights / Scheduler" row and `docs/ai/risk-register.md` "Scheduled Jobs / Freshness Detector" for full detail.
- F4 (auto-FAQ from ticket clusters): **implemented 2026-07-08**, backend and frontend complete, tested. Reused the S2 scheduler substrate and `TicketDocCoverageService` (new `findUncoveredTickets()` read) unchanged. Resolved this slice's open points: cluster threshold `FAQ_CLUSTER_SIMILARITY_THRESHOLD=0.75` + `FAQ_MIN_CLUSTER_SIZE=3` (tighter than F3's threshold — within-cluster tickets must plausibly be the same question); PII/prompt-injection handled by building the draft prompt from already-extracted, reviewer-approved ticket fields (`title`/`issueSummary`/`nextAction`), never the raw transcript. Approval materializes a draft into a normal `documents` row through the existing ingest pipeline (mirrors `scrape.processor.ts`'s pattern exactly) — hard human-approval-gate invariant, owner/admin only. S4 Insights page gained a second tab for draft review. See `docs/ai/module-ownership-map.md` "Insights / Scheduler" row and `docs/ai/risk-register.md` "Auto-FAQ Generation" for full detail.
- F5 (cross-file comparison queries): **implemented 2026-07-08**, backend complete, tested. Reused the entire S1 engine unchanged in shape: `DuckDbQueryService.runReadOnlyMultiTableQuery()` generalizes the single-table hardening to N tables, `generateMultiTableSql()` (same `'sql'` model role) can JOIN across them. New `classifyComparisonIntent()` plus 2+ dataset candidates scoring above `STRUCTURED_QUERY_COMPARISON_MIN_SCORE=0.35` routes to the multi-table flow; otherwise falls through to the existing single-dataset path unchanged. **Deviation from plan:** implemented the "multi-select disambiguation UX" as automatic natural-language detection instead of a new checkbox UI — the existing ambiguous-candidate picker and 4-state rendering already cover the UX surface needed, so no new frontend code was required; `ChatService` already renders an arbitrary-length sources array per message. No migration. See `docs/ai/module-ownership-map.md` "Datasets / Structured Query" row and `docs/ai/risk-register.md` "Structured SQL Execution" for full detail.
- F7a (confidence/coverage dashboard): **implemented 2026-07-08**, backend complete, tested. Built instead of F6 (see below) — no external-integration blocker and safe to ship with sparse real data (empty states are normal states here). Panel 1+2 (summary stats, low-score query list) are live per-request rollups over `chat_query_metrics`, no migration. Panel 3 (topic gaps) resolves this slice's open points: `TOPIC_GAP_MAX_CANDIDATES=200` rows fetched + `TOPIC_GAP_MAX_LABELED_CLUSTERS=5` LLM label calls per weekly run (the "gap-cluster labeling cost cap"); "which panels ship v1" = these three. `TopicGapProcessor` reuses `FaqClusterService.cluster()` as-is and caches labeled results in Redis (`insights:topic-gaps:${workspaceId}`, no new table, matching the plan's own "no migration" call). S4 Insights page gained a third tab. See `docs/ai/module-ownership-map.md` "Insights / Scheduler" row and `docs/ai/risk-register.md` "Coverage Dashboard" for full detail.
- F6 (Slack + email digest): **implemented 2026-07-08**, backend complete, tested. The two blockers noted above were re-checked and are NOT real: Resend is confirmed live (`RESEND_API_KEY`/`RESEND_FROM_EMAIL`/`DOMAIN` set, `EMAIL_OTP_ENABLED=true` in `.env`) — that caution was written during planning, before this was verified. The "no meaningful data yet" concern is handled, not avoided: both renderers treat a quiet week as a first-class message, same empty-state discipline as F7a. New `workspace_digest_settings` table (migration `0019`, additive). `DigestContentService` aggregates `workspace_events`/`chat_query_metrics`/`document_review_flags`/`faq_drafts`/`tickets` with zero LLM calls. Email (`NotificationsService#sendDigestEmail`) and Slack (plain `fetch()` POST, first outbound HTTP call from `apps/api`) are independent best-effort channels — one failing never blocks the other. Slack webhook URLs are validated with the existing SSRF guard (`assertPublicUrl()`) at both save and send time. Recipient scope for v1 is the workspace owner only (per-member preferences are a documented scope cut). Settings are owner/admin only for both read and write. See `docs/ai/module-ownership-map.md` "Insights / Scheduler" row and `docs/ai/risk-register.md` "Digest" for full detail.
