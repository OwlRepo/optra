...
- **F6 (Slack + email digest) deferred, not started:** two real blockers, not just plan-text caution — (1) Resend domain verification is still open in prod (sends fail today, per this doc's own reality-check section above), (2) `chat_query_metrics` has only existed since today (S3 shipped this session), so there is no meaningful multi-week trend to digest yet. Revisit once both are resolved.
