...
- PO ↔ Invoice Comparison (`purchase_orders`, `invoices`, `po_line_items`, `invoice_line_items`, `discrepancy_flags`) is a new Deep risk area as of 2026-07-09 (Optra Track A, step A1 of 3) — the first slice that persists a `DuckDbQueryService` query's output back into Postgres rather than returning an ephemeral chat answer.
  ...
  - `ProcurementParseProcessor` makes zero OpenAI calls — verified in `procurement-parse.processor.spec.ts` via a mocked-and-asserted-uncalled `@repo/ai#embedQuery` (a mechanical check, not just a design claim) — and deletes existing line items for a document before re-inserting, so a Bull job retry (`attempts:3`) never doubles rows
  ...
  - schema leaves two explicit attach points for A2 (structured PDF extraction, not built yet): `storageKey` nullable on `purchase_orders`/`invoices` (a PDF-sourced header may have no CSV object) and `sourceKind` defaulting `'csv'` on headers + line items (→ `'pdf-extraction'` later) — chosen specifically so A2 can attach without a breaking migration
