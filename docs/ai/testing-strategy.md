...
| browser e2e | `apps/e2e/tests/**/*.spec.ts` | Playwright (`apps/e2e`) | no (needs the full stack) |
...
they need a runnable Vitest spec. Browser e2e specs (`apps/e2e/tests/*.spec.ts`,
Playwright) are required by "Required test layers" below; like API e2e they need
the full stack, so they are never a RED proof.
...
**2026-09-22 — one non-reproduced `ingest.service.spec.ts` failure, recorded not chased.**
During S5 verification the full `apps/api` unit suite failed once with a single failing test in `src/ingest/ingest.service.spec.ts`, a domain S5 does not touch. Evidence gathered before moving on: the spec passes 4/4 in isolation on three consecutive runs, and the full suite passes 533/533 on four consecutive runs afterwards. The failing run was the one immediately following `bun run db:seed`, which writes the demo tenant — the same shape as the catalog reconcile flake recorded in `a2ca1cd` (cross-suite data interference, not a product defect). **Nothing was changed to make it pass**: no retry, no pool setting, no skip. If it recurs, capture the assertion text before re-running — that is the piece missing from this record.

**2026-09-22 (S6) — three lessons from the three-way engine.**
- **A test that cannot fail is not coverage.** Three of the first three-way tests passed against the *two-sided* engine, because "no flags" is also what the old engine returns for a matching pair. They were strengthened to assert `run.mode === 'three_way'` and the receipt line count, which is what actually discriminates. Check a new test fails for the right reason, not just that it fails.
- **Prove a guard discriminates by breaking it.** The POLICY v1 #9 retention test for goods receipts was verified by temporarily replacing the predicate with one that never matches, confirming red, then restoring. A guard test written after its guard proves nothing until you have seen it fail.
- **Adding an input to a query means that input must exist when it is empty.** `Papa.unparse([])` returns an empty string, so an empty `grn.csv` had no header, `read_csv_auto` built a table with no columns, and `Binder Error: Referenced column "mk" not found` broke **every** comparison — two-way included, because the third file is written on every run. The header is now written explicitly.

**2026-09-22 (S6 commit 2) — two more.**
- **A negative test is vacuous until the positive one exists.** Several needs-review tests assert that something does *not* fire (an unstated unit is not a mismatch; a case-only currency difference is not one). Those pass against an engine that does nothing at all, so they are regression guards, not proof. Only seven of the ten new assertions went red first, and that ratio is worth stating rather than reporting "10 new tests".
- **Run the engine over the demo seed and diff it.** The seeded `discrepancy_flags` are hand-written, so nothing forced them to agree with what `compare()` actually produces on the same rows. Doing that diff once, with a throwaway spec, found a receipt line's `uom` was keyed on its row number *within that receipt* — so when the two deliveries for one PO held different line counts, the same item carried two different units and the engine flagged the demo data. The engine was right; the seed was wrong. A permanent version of this check is worth having: the golden fixtures in `comparison.service.spec.ts` (`golden three-way fixture (§7.4)`) pin the engine, not the seed.
**2026-09-23 (S7 commit 1) — the timestamp that is not a tiebreak.**
- **A `defaultNow()` column written inside one transaction is the same value for every row.** `compare()` inserts all of a run's flags in a single transaction, so they share `created_at` to the microsecond. `ORDER BY created_at` over them is not a total order, and Postgres may return a different sequence for each `OFFSET` — page 2 then repeats rows from page 1 and silently drops others. The test that catches it is not "does page 1 look right" but **"are pages 1 and 2 together the whole set, with no duplicates"**: seed one comparison that yields four flags, read two pages of two, assert four distinct ids. Any paginated list in this repo needs a unique tiebreak in its `ORDER BY`, and the five existing offset services already end theirs with `desc(id)` for exactly this reason.
- **A refactor's proof is the suite it does not touch.** The current-run scope moved from a JavaScript filter into a SQL predicate. That has no failing test to write, because it is meant to change nothing — so the evidence is the four S1 current-run tests passing with **zero edits** before any pagination was added. Doing the rewrite and the shape change in one step would have destroyed that evidence; they were staged inside one commit instead, with the suite run in between.

**2026-09-23 (S7 commit 2) — a guard nobody had ever tested.**
- **Owner-vs-outsider is not a role test.** Every 403 in `apps/api/test/procurement.e2e-spec.ts` came from `WorkspaceMemberGuard`: the "outsider" was not a member of the workspace at all, so `RolesGuard` never ran. `grep "role: 'member'"` across the procurement suites returned nothing, which means a member able to dismiss and decide would have passed the entire suite. `seedMemberOfWorkspace` (idiom from `tickets.e2e-spec.ts:141-145`) closes it, and the new test asserts member-read-yes / member-write-no on the same workspace. Whenever a route is role-gated, the fixture has to be a real member of that workspace — anything else tests the wrong guard.
- **The two refusal shapes here are different and both are correct.** A caller hitting *someone else's* workspace id gets **403** from the guard. A caller hitting *their own* workspace with ids belonging to another gets **404** from the service, deliberately, so a foreign id cannot be probed for existence. A test asserting one where the other applies looks like a bug in the code and is not.
